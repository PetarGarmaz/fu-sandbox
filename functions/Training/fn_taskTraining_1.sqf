...
if((player getVariable 'FU_trainingStage') == 2) then {
	...
	waitUntil {sleep 1; _task call BIS_fnc_taskState};
	...
	waitUntil {sleep 1; count(allMapMarkers select {format["_USER_DEFINED #%1", netId player] in _x}) > 0};
...
if((player getVariable 'FU_trainingStage') == 7) then {
	...
	waitUntil{sleep 1; (getPosASL player) inArea [getPosASL FU_Target_2, 2, 2, 0, false, 2]};
...
private _task = [player, ["FU_TrainingTask_Practical_1_" + (name player), _mission], ["", "Board the helicopter", ""], FU_LZ_1, "CREATED", 1, true, "land", true] call BIS_fnc_taskCreate;

private _heli = createVehicle [_transportHeli, _spawnPos, [], 10, "FLY"];
private _heliGroup = createVehicleCrew _heli;

_heliGroup setBehaviourStrong "CARELESS";
_heliGroup setCombatMode "BLUE";
_wp = _heliGroup addWaypoint [FU_LZ_1, 50];
_heli allowDamage false;
(driver _heli) allowDamage false;
{_x setCaptive true;}foreach (units _heliGroup);

//Prevent player joining the heli group
[_mission, _heliGroup] spawn FU_fnc_trainingPreventSquadJoin;

//Prevent player from dying
[_mission] spawn FU_fnc_trainingPreventDeath;

waitUntil {sleep 1; currentWaypoint _heliGroup != _wp select 1};

_heli land "LAND";

waitUntil {sleep 1; vehicle player == _heli};
...
_wp_2 = _heliGroup addWaypoint [getMarkerPos _lz, 0];
...
waitUntil {sleep 1; currentWaypoint _heliGroup != _wp_2 select 1};

_heli land "LAND";

waitUntil {sleep 5; vehicle player != _heli};
...
private _wipeList = nearestObjects [getmarkerPos _location, ["Man"], 500, true];
